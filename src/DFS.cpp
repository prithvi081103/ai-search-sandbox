...
struct DFSSearchNode {
    std::shared_ptr<State> state;
    std::shared_ptr<DFSSearchNode> parent;
    double cost;

    DFSSearchNode(std::shared_ptr<State> s, std::shared_ptr<DFSSearchNode> p, double c)
        : state(std::move(s)), parent(std::move(p)), cost(c) {}
};

SearchResult DFS::solve(const SearchProblem& problem) {
    SearchResult result;
    ...
    std::stack<std::shared_ptr<DFSSearchNode>> frontier;
    std::unordered_set<std::string> visited;

    auto startNode = std::make_shared<DFSSearchNode>(startState, nullptr, 0.0);
    frontier.push(startNode);
    visited.insert(startState->getHash());

    std::shared_ptr<DFSSearchNode> goalNode = nullptr;
    ...
    while (!frontier.empty()) {
        ...
        for (const auto& action : successors) {
            ...
            if (visited.find(hash) == visited.end()) {
                ...
                auto childNode = std::make_shared<DFSSearchNode>(
                    action.nextState, 
                    current, 
